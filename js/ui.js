...
const UI = {
    ...
    /**
     * Actualiza la sección de Nuevos Proyectos
     * @param {Object} nuevosProyectos - Datos de Nuevos Proyectos
     */
    actualizarNuevosProyectos(nuevosProyectos) {
        const container = document.getElementById('nuevosProyectosSection');
        if (!container) return;

        if (nuevosProyectos) {
            container.style.display = 'block';
            this.actualizarTexto('npContratosEstado', nuevosProyectos.contratosEstado);
            this.actualizarTexto('npCapturados', nuevosProyectos.capturados);
            this.actualizarTexto('npFaltantes', nuevosProyectos.faltan);
            
            const porcentaje = (nuevosProyectos.porcentaje * 100).toFixed(1);
            this.actualizarTexto('npPorcentaje', porcentaje + '%');
        } else {
