...
const LectorExcel = {
    ...
    extraerIndicadoresPrime(workbook) {
        ...
        const unidades = [];
        let totales = null;
        let nuevosProyectos = null;
        let despuesDeTotales = false;
        ...
        for (const fila of datos) {
            ...
            if (unidad.toUpperCase().includes('TOTAL')) {
                totales = registro;
                despuesDeTotales = true;
            } else if (despuesDeTotales && unidad.toUpperCase().includes('NUEVOS PROYECTOS')) {
                // Guardar referencia pero NO agregar aún (se agrega después con datos de FALTAN)
                nuevosProyectos = registro;
            } else if (!despuesDeTotales) {
                unidades.push(registro);
            }
        }

        return { unidades, totales, nuevosProyectos };
    },
